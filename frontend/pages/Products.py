...
from frontend.controller.products import get_products
...
async def products_page() -> None:
    ...
    with st.spinner("Loading the product list..."):
        ...
        response = await get_products(search, sort_key, app_pet, category, subcategory, page_size, page)
        ...
        with expander:
            ...
            with controls[0]:
                st.markdown("**Sort by**")
                sort = st.selectbox("Sort by:", sort_options, key="sort", label_visibility="collapsed")  # noqa
            with controls[1]:
                st.markdown("**Category**")
                category = st.multiselect("Category", options=categories, key="category", label_visibility="collapsed")  # noqa
                st.markdown("**Subcategory**")
                subcategory = st.multiselect(  # noqa
                    "SubCategory", options=subcategories, key="subcategory", label_visibility="collapsed"
                )
            with controls[2]:
                st.markdown("**Applicable Pet**")
                app_pet = st.multiselect(  # noqa
        ...
        bottom = st.columns(7)
        with bottom[6]:
            page_select = st.selectbox("Page Number", range(1, num_batches + 1), key="page", disabled=num_batches == 1)  # noqa

    st.markdown(FOOTER, unsafe_allow_html=True)


asyncio.run(products_page())
